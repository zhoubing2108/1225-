...
const showRevert = (e) => {
  console.log('归还页的内容');
  alert('归还', '是否已经确认归还', [
    { text: 'Cancel', onPress: () => console.log('不执行操作') },
  ...
  render() {
    ...
    const NeedList = () => (
      needList.map(e => (<div key={e.flow.id} style={{ fontSize: 16, padding: '10px', background: '#eefaff', border: '1px solid #bbe1f1' }}>
        ...
        {e.btn == 'cancel' ? <Button onClick={() => showRevert(e)} type='primary' size='small' style={{ display: 'inline-block', height: 24, lineHeight: '24px', margin: '5px 10px 0 0' }}>归还</Button> : null}
      ...
      <Fragment>
        <div style={{ marginTop: '5px', }}>
          <Tabs tabs={tabs} style={{ width: '100%' }} initialPage={0} animated={false} useOnPan={false}>
            ...
            <div style={{ height: '100%', backgroundColor: '#fff' }}>
              {/* <span>test<Button onClick={() => showRevert()}>归还</Button></span> */}
              <HistoryList />
